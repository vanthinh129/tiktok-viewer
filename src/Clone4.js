...
const BrowserService = require("./BrowserService.old")
    ...
    try {
        ...
        switch (type) {
            ...
            case "enter":
                ...
                // url = `https://webcast.us.tiktok.com/webcast/room/enter/?aid=1988&app_language=en&app_name=tiktok_web&browser_language=en-US&browser_name=Mozilla&browser_online=true&browser_platform=MacIntel&browser_version=${encodeURIComponent(appVersionDefault)}&channel=tiktok_web&cookie_enabled=true&device_id=${device_id}&device_platform=web_pc&device_type=web_h264&focus_state=true&from_page=user&history_len=4&is_fullscreen=false&is_page_visible=true&os=mac&priority_region=&referer=&region=US&screen_height=900&screen_width=1440&tz_name=Asia%2FSaigon&webcast_language=en&msToken=${msToken}`
                _bodyJson = {enter_source: "others-others", room_id: room_id}
                break;
...
fetch() {
    return new Promise(async r=>{
        ...
        try {
            ...
            let  { url: targetUrl, xbogus, _signature} = await br.buildUrlPageFull({url: `${route}?${endpoint}`})
