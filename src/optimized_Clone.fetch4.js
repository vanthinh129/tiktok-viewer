...
// Singleton pattern cho BrowserService
...
// Sử dụng WeakMap để quản lý tài nguyên và tránh rò rỉ bộ nhớ
...
class OptimizedClone {
  constructor({ cookie_string, task_id, room_id, proxy, proxy_list }) {
    ...
    this.device_id = helper.getString(cookie_string, ';wid=', ';') || ("7284943" + random_de);
    ...
    // Cache cho HTTP request
    this._requestCache = new Map();
    
    // Giới hạn số lượng request cache để tránh rò rỉ bộ nhớ
  ...
  async run() {
    ...
    try {
      ...
      console.error(`[${helper.getTime()}] Error in run for ${this.username}:`, err.message);
      this.status = "end";
      return false;
    }
  }
  ...
  async runJoin() {
    try {
      ...
      console.error(`[${helper.getTime()}] Error joining room for ${this.username}:`, err.message);
      return false;
    }
  }
  ...
  async runFetchs() {
    ...
    while (is_run) {
      try {
        ...
        console.error(`[${helper.getTime()}] Error in fetch loop for ${this.username}:`, err.message);
  ...
  async callApi({ type }) {
    ...
    try {
      ...
      const br = await BrowserService.getInstance(userAgentDefault, {
        initSign: true, 
        headless: "yes"
      });
      ...
      const { url: targetUrl, xbogus, _signature, bodyEncoded } = await br.buildUrlPageFull({
        url,  
        bodyJson: _bodyJson, 
        msToken
      });
      ...
      return { is_403: false, data: data_page.body, body: data_page.body };
      ...
      if (proxy) {
        ...
        if (typeof proxy == "string") {
          ...
          if (!proxy.includes("https") && !proxy.includes("http")) {
            const parsedProxy = helper.parserProxyString(proxy);
  ...
  _cleanupRequestCache() {
    ...
    // Chỉ giữ lại 10 requests gần nhất
    if (this._requestCache.size > 10) {
      const keysToDelete = Array.from(this._requestCache.entries())
        .sort((a, b) => a[1].timestamp - b[1].timestamp)
        .slice(0, this._requestCache.size - 10)
        .map(entry => entry[0]);
  ...
  fetch() {
    return new Promise(async r => {
      ...
      try {
        ...
        const { cookie_string, video_id, msToken: cmsToken, session_id: csession_id } = this;
        const msToken = cmsToken || getString(cookie_string + ';', 'msToken=', ';');
        const session_id = csession_id || getString(cookie_string.replace(/ /g,'') + ';', ';sessionid=', ';') ||
        ...
        // Lấy browser service
        const BrowserService = await getBrowserService();
        const br = await BrowserService.getInstance(userAgentDefault, { initSign: true, headless: "yes" });
        ...
        const { url: targetUrl, xbogus, _signature } = await br.buildUrlPageFull({
          url: `${route}?${endpoint}`, 
          msToken
        });
        ...
        // Xử lý binary response nếu có
        if (data_page.body) {
          const hex = data_page.bodyBinary ? data_page.bodyBinary.toString('hex') : '';
          const idHexServer = helper.getString(hex, "a181", "6720") || 
        ...
        if (data_page.headers && data_page.headers['set-cookie'] && data_page.headers['set-cookie'].length) {
          const new_cookie = data_page.headers['set-cookie'].map(i => {
            const msToken = getString(i, 'msToken=', ';');
            if (msToken) {
        ...
        this.fetch_403 = error.message == "Request failed with status code 403";
        return r(result);
      }
    });
  }
