...
class GroupView {
    ...
    static startProxyGroupViewers({accounts, task_id, proxy, room_id, ex_wrss, rabbitService, server_site}) {
        try{
                ...
                let config_data = { cookie_string: i, proxy_string: proxy || p, useragent: helper.genuaMAC(),rooms: {}, task_id, isShowLog:  false, clone, ex_wrss, wrss_obejct, rabbitService, server_site}
    ...
    static async startViewers({accounts, task_id, proxy, room_id, ex_wrss, tokens, rabbitService, server_site}) {
        try{
            let total = 0 
            console.log("Start task_id:",task_id, " room:", room_id," accounts:", accounts.length)
            data[task_id] = { sockets: []};
            let grouped_proxy = accounts.reduce((pre,cur)=>{
                let p = helper.getString(cur+";", "proxy_socket=",";")
                return {...pre, [p]: pre[p] ? [...pre[p], cur]: [cur]}
            },{})
            // console.log(grouped_proxy)
            for(let i in grouped_proxy){
                // let adn = await helper.getLocationProxy(i)
                console.log("Start group", i)
                GroupView.startProxyGroupViewers({accounts:grouped_proxy[i] , task_id, proxy, room_id, rabbitService, server_site})
