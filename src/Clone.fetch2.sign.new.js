...
const {sign, xgnarly} = require("./sign_all")
...
class Clone {
  ...
  async callApi({ type }) {
    ...
    try {
        ...
        // let br = await BrowserService.getInstance(userAgentDefault, {initSign: true, headless: "no"})
        ...
        // let targetUrl =  sign(url,bodyEncoded,msToken)

        // console.log("targetUrl",targetUrl)
        // targetUrl = targetUrl.split("&X-Gnarly")[0]
        // console.log("targetUrl",targetUrl)
        // let new_param = targetUrl.split("?")[1]
        // let my_xgnarly = xgnarly({queryString:new_param, body:querystring.stringify(_bodyJson), userAgent: userAgentDefault})
        // console.log("xgnarly",my_xgnarly)
        // targetUrl += `&X-Gnarly=${my_xgnarly}`
        // console.log("targetUrl",targetUrl)
        // process.exit(1)
        // let  { url: targetUrl, xbogus, _signature, bodyEncoded} =  sign({url, body: _bodyJson, msToken,userAgent: userAgentDefault})
        // let data_sign =  sign({url, body: _bodyJson, msToken,userAgent: userAgentDefault})
        let bodyEncoded = querystring.stringify(_bodyJson)
        let {targetUrl} =  await signreq({url, bodyEncoded, msToken,userAgent: userAgentDefault})
        // console.log("data_sign",data_sign3)
        // console.log("xbogus",helper.getString(targetUrl, 'X-Bogus=', '&'))
        // console.log("xbogus2",helper.getString(data_sign.url, 'X-Bogus=', '&'))
        // console.log("xbogus3",helper.getString(data_sign3.url, 'X-Bogus=', '&'))
        // targetUrl = data_sign3.url
        console.log("targetUrl",targetUrl)
...
fetch() {
    return new Promise(async r=>{
        ...
        try {
            ...
            let  { targetUrl} =  await signreq({url:`${route}?${endpoint}`, bodyEncoded:'',msToken,userAgent: userAgentDefault})
