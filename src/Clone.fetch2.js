...
class Clone {
  constructor({ cookie_string, task_id, room_id,proxy, proxy_list }) {
    ...
    this.msToken = getString(cookie_string + ';', 'msToken=', ';');
  ...
  async callApi({ type }) {
    ...
    // let screen_height = 470
    // let screen_width = 620
    ...
    try {
        ...
        switch (type) {
            ...
            case "enter":
                ...
                url = `https://webcast.tiktok.com/webcast/room/enter/?aid=1988&app_language=vi-VN&app_name=tiktok_web&browser_language=vi&browser_name=Mozilla&browser_online=true&browser_platform=MacIntel&browser_version=${this.encodeRFC3986URIComponent(appVersionDefault)}&channel=tiktok_web&cookie_enabled=true&data_collection_enabled=true&device_id=${device_id}&device_platform=web_pc&device_type=${device_type}&focus_state=true&from_page=&history_len=4&is_fullscreen=false&is_page_visible=true&os=mac&priority_region=VN&referer=https%3A%2F%2Fwww.tiktok.com%2F&region=VN&root_referer=https%3A%2F%2Fwww.tiktok.com%2F&screen_height=${screen_height}&screen_width=${screen_width}&tz_name=Asia%2FBangkok&user_is_login=true&verifyFp=${verifyFp}&webcast_language=vi-VN`
      ...
      // url = url.replaceAll('Asia/Saigon','America/Chicago').replaceAll('Asia%2FBangkok','America%2FChicago').replaceAll('vi-VN','en-US').replaceAll('=vi','=en')
           let target_url = ""
     
      
           let  { url: targetUrl, xbogus, _signature, bodyEncoded} = await br.buildUrlPageFull({url,  bodyJson: _bodyJson, msToken})
            target_url = targetUrl
        let s_sdk_crypt_sdk=helper.getString(cookie_string, 'crypt_sdk_b64=', ';');
        let s_sdk_sign_data_key= helper.getString(cookie_string, 'sign_data_key_b64=', ';');
      ...
      request(options,(error, response, body) => {
        if(!body){
          that.status_viewer = 6
            ...
            //     if(cookies[i].includes("msToken")){
            //       let msTokenNew = helper.getString(cookies[i], "msToken=", ";")
            //       this.msToken = msTokenNew
            //       this.cookie_string = this.cookie_string.replace(/msToken=[^;]+/g, `msToken=${msTokenNew}`);
            //       console.log(this.username,"msTokenNew", msTokenNew)
            //     }
            //   }
            // }
          if(!done){
...
fetch() {
    return new Promise(async r=>{
        ...
        try {
            ...
            // endpoint = endpoint.replaceAll('Asia/Saigon','America/Chicago').replaceAll('vi-VN','en-US').replaceAll('=vi','=en')
