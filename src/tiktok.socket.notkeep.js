...
class TikTokSocket {
    constructor(data) {
        let { cookie_string, proxy_string, useragent, isShowComment, onMessage, isShowLog, reconnect_after_time, task_id, server_site } = data;
        ...
        this.closed = false;
        this.is_101 = false;
        this.reconnect_after_time = reconnect_after_time || this.getRandomInt(3*60, 7*60)*1000
        ...
        let result = await new Promise(async(r) => {
            try {
                ...
                client.on('connect',async function (connection) {
                    ...
                    that.disconnect();
                    await helper.delay(that.getRandomInt(20000, 40000))
                    return r(false)
