...
class TikTokSocket {
    ...
    async connect({ room_id }) {
        ...
        //     // that.is_101 && 
            !this.wrss
             // ||  this.lastFetch < (Date.now() - 5 * 60000)
             ) {
            ...
            try {
                ...
                client.on('connect',async function (connection) {
                    ...
                    // that.client.socket._readableState.length = 18032903
                            await that.sendAsync(Buffer.from('3A026862', 'hex'));
    ...
    sendAsync(data) {
        return new Promise((resolve, reject) => {
            if (this.connection.socketConnected) {
                this.connection.sendBytes(data, function(err) {
                    if (err) {
                        // reject(err);
                        resolve();
                    } else {
