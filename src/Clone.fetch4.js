...
class Clone {
  ...
  async run1(){
    ...
    async runJoin(){
        let cookie_status = await helper.checkCookieLive({ username: this.username, cookie_string: this.cookie_string, proxy:this.proxy, proxy_list: this.proxy_list }); 
...
async makeRequest (options) {
    ...
    if(!url.includes("X-Gnarly") ){
        return  r({error:"None sign"+that.proxy, body: "", headers:  {}, status:  null })
    }
    method = method || "get"
    ...
    let head = await new Promise(r => {
      ...
      if(form) requestOptions.form = form;
      let done = false;
      setTimeout(() => {
        if(!done){
          done = true;
      ...
      // Kiểm tra xem có sử dụng proxy hay không
      // Chỉ khi proxy là false thì mới không dùng this.proxy
      if (proxy !== false) {
        let proxystr = "";
        // Ưu tiên sử dụng this.proxy nếu không có proxy được chỉ định rõ
        let proxyToUse = proxy || that.proxy;
        
        if (typeof proxyToUse == "string") {
          proxystr = proxyToUse;
          if(!proxyToUse.includes("https") && !proxyToUse.includes("http")){
            let { protocol, host, port, username, password } = helper.parserProxyString(proxyToUse);
            proxystr = `${protocol || "http"}://${username && password ? `${username}:${password}@` : ''}${host}:${port}`;
          }
        } else if (proxyToUse) {
          let { protocol, host, port, username, password } = proxyToUse;
          proxystr = `${protocol || "http"}://${username && password ? `${username}:${password}@` : ''}${host}:${port}`;
        }
        
        if (proxystr) {
          requestOptions.proxy = proxystr;
        }
      }
