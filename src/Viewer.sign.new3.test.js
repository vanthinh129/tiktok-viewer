...
const { delay, getString, parserProxyString, getTime, getRandomInt } = require("./helper");
...
class GroupView {
    ...
    async callApi({ type }) {
        ...
        try {
            ...
            let {url: targetUrl} = await this.signreq({url, body: _bodyJson, msToken, userAgent: userAgentDefault});let is_retry = false;
            // console.log("targetUrl", targetUrl);
            // let  { url: targetUrl, xbogus, _signature, is_retry} = await br.buildUrlPageFull({url,  bodyJson: _bodyJson, msToken})
    ...
    async signreq(options) {
        ...
        if (data && data.data) {
            let {targetUrl, userAgent,appVersion,msToken} = data.data;
            return {
                url: targetUrl,
            }
        }else{
            console.log("Error signing request:", data);
            return { url: "", xbogus: "", _signature: "", is_retry: false };
        }
    }
    ...
    fetch() {
        return new Promise(async r => {
            ...
            try {
                ...
                let {url: targetUrl} = await this.signreq({
                    url: `${route}?${endpoint}`,
                    msToken,
