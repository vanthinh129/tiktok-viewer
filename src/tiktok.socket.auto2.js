...
class TikTokSocket {
    constructor(data) {
        let { cookie_string, proxy_string, useragent, isShowComment, onMessage, isShowLog, reconnect_after_time, task_id, server_site, wrss } = data;
    ...
    async connect({ room_id }) {
        ...
        let time_get_wrss_next = 4*60*1000;//that.getRandomInt(60, 5*60)*1000
        time_get_wrss_next = that.getRandomInt(30, 3*60)*1000
        let time_now = Date.now();
        if(
        //     // that.is_101 && 
            !this.wrss
             ||  this.lastFetch < (time_now - time_get_wrss_next)
        ) {
            let res = await this.clone.fetch()
            this.lastFetch = time_now;
        ...
        if (!result) {
            if(this.closed){
                ...
                this.retryTime++;
                this.retryTimeFull++;
                let randomNum = that.getRandomInt(3000, 10000)
