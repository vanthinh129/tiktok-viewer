...
class TikTokSocket {
    ...
    constructor(data) {
        ...
        // Track current room_id
        this.currentRoomId = null;
        this.roomChangeInterval = null;
    ...
    async checkRoomChange(room_id){
        ...
        */
        try{
            if(fs.existsSync(file_room_change_check)){
                ...
                if(data[room_id] && data[room_id] != room_id){
                    if(that.isShowLog) {
                        console.log(`Room change detected: ${room_id} -> ${data[room_id]}`);
                    }
                    return data[room_id]; // Return new room_id
                }
            }
        }catch(e){
        ...
        return null; // No room change
    }

    // Remove processed room change entry to prevent infinite loop
    removeProcessedRoomChange(oldRoomId, newRoomId) {
        let that = this;
        let file_room_change_check = path.resolve("./room_change_check.json");
        ...
        try {
            if(fs.existsSync(file_room_change_check)) {
                ...
                if(data[oldRoomId] === newRoomId) {
                    delete data[oldRoomId];
                    fs.writeFileSync(file_room_change_check, JSON.stringify(data, null, 2));
                    
                    if(that.isShowLog) {
                        console.log(`Removed processed room change: ${oldRoomId} -> ${newRoomId}`);
                    }
                }
            }
        } catch(e) {
            console.log("removeProcessedRoomChange error", e);
        }
    }

    // Start periodic room change checking
    startRoomChangeMonitoring(room_id, intervalMs = 30000) {
        let that = this;
        
        // Set current room_id
        this.currentRoomId = room_id;
        
        if(this.roomChangeInterval) {
            clearInterval(this.roomChangeInterval);
        }
        
        this.roomChangeInterval = setInterval(async () => {
            if(!that.socketConnected || that.closed) {
                clearInterval(that.roomChangeInterval);
                that.roomChangeInterval = null;
                return;
            }
            
            try {
                const newRoomId = await that.checkRoomChange(that.currentRoomId);
                if(newRoomId) {
                    const oldRoomId = that.currentRoomId; // Save old room_id before update
                    if(that.isShowLog) {
                        console.log(`Auto room change detected: ${oldRoomId} -> ${newRoomId}`);
                    }
                    const success = await that.switchRoom(newRoomId);
                    if(success) {
                        that.currentRoomId = newRoomId; // Update current room_id
                        // Remove processed entry from file to avoid infinite loop
                        that.removeProcessedRoomChange(oldRoomId, newRoomId);
                    } else {
                        // If switch failed, remove entry to avoid infinite retries
                        console.log(`Switch room failed, removing entry to prevent infinite loop`);
                        that.removeProcessedRoomChange(oldRoomId, newRoomId);
                    }
                }
            } catch(e) {
                console.log("Room change monitoring error:", e);
            }
        }, intervalMs);
        
        if(that.isShowLog) {
            console.log(`Room change monitoring started for room: ${room_id} (interval: ${intervalMs}ms)`);
        }
    }

    // Stop room change monitoring
    stopRoomChangeMonitoring() {
        if(this.roomChangeInterval) {
            clearInterval(this.roomChangeInterval);
            this.roomChangeInterval = null;
            if(this.isShowLog) {
                console.log("Room change monitoring stopped");
            }
        }
        // Don't reset currentRoomId here as we might still need it
    }

    // Get current room ID being monitored
    getCurrentRoomId() {
        return this.currentRoomId;
    }

    // Force check room change manually
    async forceCheckRoomChange() {
        if(!this.currentRoomId) {
            console.log("No current room to check");
            return null;
        }
        
        const newRoomId = await this.checkRoomChange(this.currentRoomId);
        if(newRoomId) {
            if(this.isShowLog) {
                console.log(`Manual room change detected: ${this.currentRoomId} -> ${newRoomId}`);
            }
            const oldRoomId = this.currentRoomId;
            const success = await this.switchRoom(newRoomId);
            if(success) {
                this.currentRoomId = newRoomId;
                // Remove processed entry
                this.removeProcessedRoomChange(oldRoomId, newRoomId);
            } else {
                // Remove entry even if failed to prevent infinite retries
                console.log(`Manual switch room failed, removing entry to prevent infinite loop`);
                this.removeProcessedRoomChange(oldRoomId, newRoomId);
            }
            return newRoomId;
        }
    ...
    async switchRoom(room_id){
        ...
        try {
            ...
            await this.clone.callApi({type: "enter"});
            ...
            if(this.socketConnected && this.connection) {
                const switchRoomMessage = this.messageBuilder.createSwitchRoomMessage(room_id, {
                    cursor: this.cursor || '',
                    liveId: '12',
                    identity: 'audience',
                    accountType: '0'
                ...
                this.connection.sendBytes(switchRoomMessage);
                if(that.isShowLog) {
                    console.log(`Switch room message sent for room: ${room_id}`);
                    console.log(`Switch room message (base64): ${switchRoomMessage.toString('base64')}`);
                }
            }
    ...
    async connect({ room_id }) {
        ...
        // Check for room change first
        const newRoomId = await this.checkRoomChange(room_id);
        if(newRoomId) {
            room_id = newRoomId; // Use new room_id
        }
        
        // If already connected and just need to switch room
        if(this.socketConnected && this.connection) {
            if(that.isShowLog) {
                console.log(`Already connected, switching to room: ${room_id}`);
            }
            const success = await this.switchRoom(room_id);
            if(success) {
                this.currentRoomId = room_id;
            }
            return success;
            ...
            try {
                ...
                client.on('connect', async function (connection) {
                    ...
                    const createMessageClientHBFirst = () => {
                        ...
                        // Use current room_id instead of closure room_id
                        const currentRoom = that.currentRoomId || room_id;
                        const heartbeatMessage = that.messageBuilder.createHeartbeatMessage(currentRoom);
                    ...
                    const createImEnterHexMessage = () => {
                        ...
                        // Use current room_id instead of closure room_id
                        const currentRoom = that.currentRoomId || room_id;
                        const enterRoomMessage = that.messageBuilder.createEnterRoomMessage(currentRoom, {
                            cursor: that.cursor || '',
                            liveId: '12',
                            identity: 'audience',
                            accountType: '0'
                    ...
                    console.log("enter_mess", enter_mess.toString("base64"))
                    _sendPing2()
                    
                    // Set current room_id and start monitoring
                    that.currentRoomId = room_id;
                    that.startRoomChangeMonitoring(room_id, 30000);
