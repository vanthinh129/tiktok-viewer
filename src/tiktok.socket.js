...
class TikTokSocket {
    ...
    async connect({ room_id }) {
        ...
        let clone = new Clone({cookie_string: this.cookie_string, room_id, proxy: p,proxy_list})
        if(
            // that.is_101 && 
            !this.wrss) {
            let res = await clone.fetch()
            this.wrss = clone.wrss
            if(!this.wrss){
                this.wrss = this.generateRandomString(43)
            }
        }
