/**
 * File: OptimizedViewer.combined.js
 * Kết hợp các chức năng tối ưu của Clone.fetch5.js và optimized_Viewer.new.js
...
// Biến toàn cục để theo dõi số lượng instance đã tạo
let totalInstancesCreated = 0;
const MAX_INSTANCES = 500;

// Hàm khởi tạo OptimizedClone với cơ chế kiểm tra sức khỏe
const createOptimizedClone = async (options) => {
  // Kiểm tra giới hạn số lượng instance
  if (totalInstancesCreated >= MAX_INSTANCES) {
    console.error(`[${getTime()}] Đã đạt giới hạn số instance tối đa (${MAX_INSTANCES})`);
    return null;
  }
  
  try {
    // Kiểm tra tham số đầu vào
    if (!options || !options.cookie_string || !options.room_id) {
      throw new Error("Thiếu tham số bắt buộc: cookie_string hoặc room_id");
    }
    
    // Tạo instance mới với xử lý lỗi
    const clone = new OptimizedClone(options);
    totalInstancesCreated++;
    
    // Thiết lập hệ thống theo dõi lỗi
    setupErrorMonitoring(clone);
    
    return clone;
  } catch (error) {
    console.error(`[${getTime()}] Lỗi khi tạo OptimizedClone: ${error.message}`);
    return null;
  }
};

// Theo dõi lỗi và khôi phục tự động
const setupErrorMonitoring = (instance) => {
  // Thêm hệ thống báo cáo lỗi
  const originalRun = instance.run;
  
  instance.run = async function() {
    try {
      // Ghi log bắt đầu
      console.log(`[${getTime()}] Bắt đầu chạy cho ${this.username} vào phòng ${this.room_id}`);
      
      // Theo dõi usage memory
      const memUsageBefore = process.memoryUsage();
      
      // Chạy hàm gốc
      const result = await originalRun.apply(this, arguments);
      
      // Ghi log khi hoàn thành thành công
      const memUsageAfter = process.memoryUsage();
      const memDiff = (memUsageAfter.heapUsed - memUsageBefore.heapUsed) / 1024 / 1024;
      
      console.log(`[${getTime()}] Kết thúc chạy cho ${this.username}, Memory +${memDiff.toFixed(2)} MB`);
      
      return result;
    } catch (error) {
      // Xử lý lỗi
      console.error(`[${getTime()}] Lỗi trong quá trình chạy ${this.username}: ${error.message}`);
...
// Hàm dọn dẹp tài nguyên toàn cục
const cleanupResources = async () => {
  // Giải phóng tất cả browser instances
  ...
  console.log(`[${getTime()}] Đã dọn dẹp tất cả tài nguyên`);
};

// Export thêm các hàm tiện ích
module.exports.createOptimizedClone = createOptimizedClone;
module.exports.cleanupResources = cleanupResources;