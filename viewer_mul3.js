...
async function runViewer(room_id){
    ...
    let acc_string = await helper.strData(path.resolve("./data_test/acc_socket_edu.txt"));
    ...
    for(let index = 0 ; index < splice_accounts.length; index ++ ){
        ...
        accs = accs.map(account => {
            ...
            // account = account +";proxy="+proxy+";proxy_socket=usdutvin:suwser5mup15@23.95.150.145:6114;"
            account = account +";proxy="+proxy+";proxy_socket=us060-35382:NOGFE@sp22v5-us01.proxy.mkvn.net:17510;"
            //
            return account
        });
